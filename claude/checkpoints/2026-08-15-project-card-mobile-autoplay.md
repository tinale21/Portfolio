...
- Same underlying pattern (`autoPlay muted loop playsInline` with no JS-driven fallback) is used across every case-study Hero/section video in the codebase (AIG, Wayve, Emora, Framer Redesign — `grep -l autoPlay src/components/case-studies` turns up ~11 files). Only fixed the one the user actually flagged (home page Selected Projects); if the same play-button symptom shows up on any case-study page, the identical `IntersectionObserver` fix would apply there too.
- Could not verify on an actual mobile device/browser from this environment — headless Chrome's autoplay policy doesn't reproduce the reported bug, so this fix is based on established best practice for this exact class of issue, not a confirmed-fixed repro.
